...
`.env` at the project root is loaded by [src/settings/config.py](src/settings/config.py) into a pydantic-settings `Config` singleton imported as `from src.settings.config import config`. `GROQ_API_KEY` is **required** — importing anything that transitively imports `config` raises a validation error without it. See [.env.example](.env.example) for the full set.
